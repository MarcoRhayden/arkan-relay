...
// -------------------------------------------------------------------------------------------------
#if defined(_WIN32)
static void configure_console_colors_(
    const std::shared_ptr<spdlog::sinks::wincolor_stdout_sink_mt>& sink)
{
...
#else
static void configure_console_colors_(
    const std::shared_ptr<spdlog::sinks::ansicolor_stdout_sink_mt>& sink)
{
...
void Logger_Spdlog::init(const arkan::relay::domain::Settings& s)
{
  ...
  const fs::path sockPath =
      dir / (s.socketLogFilename.empty() ? "relay_socket.log" : s.socketLogFilename);
  ...
  auto app_file =
      std::make_shared<spdlog::sinks::rotating_file_sink_mt>(appPath.string(), 5 * 1024 * 1024, 3);
  auto sock_file =
      std::make_shared<spdlog::sinks::rotating_file_sink_mt>(sockPath.string(), 5 * 1024 * 1024, 3);
  ...
  if (use_async)
  {
    try
    {
      spdlog::init_thread_pool(qsize, workers);
    }
    catch (...)
    { /* already initialized */
    }
