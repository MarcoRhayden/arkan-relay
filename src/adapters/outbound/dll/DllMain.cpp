...
static void EnsureConsole_New() {
  ...
  HANDLE hErr = CreateFileW(L"CONOUT$", GENERIC_READ | GENERIC_WRITE,
                            FILE_SHARE_READ | FILE_SHARE_WRITE, nullptr,
                            OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr);
  ...
  FILE* outF = nullptr; FILE* errF = nullptr; FILE* inF = nullptr;
...
static LONG WINAPI CrashFilter(EXCEPTION_POINTERS* ep) {
  ...
  std::snprintf(buf, sizeof(buf),
                "ArkanRelay: CRASH code=0x%08lX at=%p\r\n",
...
static DWORD RunRelayCore() {
  ...
  if (!has(s.fnSendAddr) || !has(s.fnRecvAddr) || !has(s.fnSeedAddr) || !has(s.fnChecksumAddr)) {
    OutputDebugStringA("ArkanRelay: missing required addresses (send/recv/seed/checksum) - aborting\r\n");
    return 0;
  }
